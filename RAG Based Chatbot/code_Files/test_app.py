...
# We need to mock vector_store.load_faiss_index BEFORE importing app
# because app.py calls it at the top level
sys.modules['vector_store'] = MagicMock()
sys.modules['vector_store'].load_faiss_index.return_value = (MagicMock(), [], {})

# Mock retrieval to avoid needing real index
...
# Mock embeddings to prevent heavy model load
...
class TestMilestone3(unittest.TestCase):
    ...
    def test_multi_turn_conversation(self, mock_create):
        ...
        # Verify logic passed history to OpenAI
        # Get the arguments of the LAST call to create
        call_args = mock_create.call_args
        messages = call_args[1]['messages']
        
        # Check system prompt or user prompt for history presence
